...
def applyCollision(gMap, player, direction):
    ...
    for y in range(len(gMap)):
        for x in range(len(gMap[0])):
            if (
                gMap[y][x].x in betweenBlock
                and y in betweenBlock2
                and gMap[y][x].blockType in COLLISION_BLOCK
            ):
                player.x += direction * PLAYER_SPEED
            elif (
                gMap[y][x].x in betweenBlock
                and y in betweenBlock2
                and gMap[y][x].blockType in INTERACTIVE_BLOCK.keys()
            ):
                for effect in INTERACTIVE_BLOCK[gMap[y][x].blockType]:
                    if effect == "bounce":
                        ...
                        player.score += 1
            if (
                gMap[y][x].y == betweenBlock2[1]
                and gMap[y][x].x in betweenBlock
                and gMap[y][x].blockType in COLLISION_BLOCK
            ):
